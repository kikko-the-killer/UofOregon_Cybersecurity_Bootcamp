...
Explain that the number one item on this list is **injection**, which will be the primary focus in today's class.
...
Remind the class that so far we have covered the following concepts:
  ...
  - **Injection**, the number one web application vulnerability on the OWASP Top 10, happens when an application takes untrusted input that alters the way a program is intended to function.
...
Explain that **cross-site scripting**, or **XSS**,  is another injection type that occurs when an application takes in malicious user input that modifies the source code of the application.
